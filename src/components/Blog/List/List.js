...
class List extends Component {
  ...
  componentDidMount() {
    axios.get('https://raw.githubusercontent.com/lzm854676408/big-demo/master/posts/index.json')
    .then((res) => {
       this.setState({
         posts: res.data,
         wait:false
        ...
        map((b) =>  {
          ...
          for (var key in posts) {
            if (query.test(posts[key].title)) {
              blogCards.push(<BlogCard title={posts[key].title} date={posts[key].date} index={posts[key].index} url={b.name} key={Math.random()}/>)
          }
        }
      }
    return(
